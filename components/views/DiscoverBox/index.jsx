  ...
  [
    ...
    },
    {
      ...
      render: (text, record) => (
        <button
          type="button"
          class=" border bg-gradient-to-r from-[#D71C5D] to-[#FF9017] hover:from-[#D71C5D]/90 hover:to-[#FF9017]/90 focus:outline-none text-[#fff] focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center"
        >
          <svg
            className="mr-1"
            stroke="currentColor"
...
const DiscoverBox = () => {
  ...
  const fetchJson = () => {
    fetch("/data/data.json")
      ...
      .then((data) => {
        setDataNftFree(data.slice(0, 10));
        setTotalRow(data.slice(0, 10).length);
      })
      .catch((error) => {
  ...
  return (
    <LayoutMain>
      ...
      <div></div>
      {/* <Pagination current={1} pageSize={2} total={10} onClick={handleClickPagination} /> */}
    </LayoutMain>
  );
};
